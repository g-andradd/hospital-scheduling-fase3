...
|---:|---|---|---|---|---|
| 1 | 1:00 | Autenticar como enfermeiro e criar uma consulta futura | http://localhost:8081/swagger-ui.html — `POST /auth/login` e depois `POST /api/v1/consultas` | `accessToken`, `expiresIn` e `perfil`; a consulta volta `201` com `id` e `status: AGENDADA` | Se o Swagger demorar, mostre o `consultaId` que o `make demo` já imprimiu e siga para o bloco 2 |
